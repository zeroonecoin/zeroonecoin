...
pkgdatadir = $(datadir)/zeroonecore
pkgincludedir = $(includedir)/zeroonecore
pkglibdir = $(libdir)/zeroonecore
pkglibexecdir = $(libexecdir)/zeroonecore
...
DIST_COMMON = $(srcdir)/Makefile.in $(srcdir)/Makefile.am \
	...
	$(top_srcdir)/src/config/zeroone-config.h.in \
...
CONFIG_HEADER = $(top_builddir)/src/config/zeroone-config.h
...
OTOOL64 = 
PACKAGE = zeroonecore
PACKAGE_BUGREPORT = https://github.com/zeroonecoin/zeroone/issues
PACKAGE_NAME = ZeroOne Core
PACKAGE_STRING = ZeroOne Core 0.12.1
PACKAGE_TARNAME = zeroonecore
PACKAGE_URL = 
...
BITCOIN_QT_BIN = $(top_builddir)/src/qt/zeroone-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/zeroone-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
OSX_APP = ZeroOne-Qt.app
OSX_DMG = ZeroOne-Core.dmg
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_zeroone_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_zeroone_coverage.info test_zeroone.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/ZeroOne-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/zeroone-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/zeroone-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/zeroone-config.h
$(top_srcdir)/src/config/zeroone-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/zeroone-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/ZeroOne-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/ZeroOne-Qt

$(OSX_DMG): $(APP_DIST_EXTRAS)
	$(GENISOIMAGE) -no-cache-inodes -D -l -probe -V "ZeroOne-Core" -no-pad -r -dir-mode 0755 -apple -o $@ dist
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/ZeroOne-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_zeroone.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_zeroone -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_zeroone_filtered.info: test_zeroone.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_zeroone_filtered.info
#	$(MKDIR_P) qa/tmp
...
#rpc_test.info: test_zeroone_filtered.info
...
#test_zeroone_coverage.info: baseline_filtered_combined.info test_zeroone_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_zeroone_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_zeroone_filtered.info block_test_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_zeroone_filtered.info -a block_test_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_zeroone.coverage/.dirstamp:  test_zeroone_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_zeroone.coverage/.dirstamp total.coverage/.dirstamp

# ZeroOne: disable the test until upstream switches from convoluted comparison tool
...
clean-local:
	rm -rf coverage_percent.txt test_zeroone.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
