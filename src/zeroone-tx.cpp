...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = _("ZeroOne Core zeroone-tx utility version") + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  zeroone-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded zeroone transaction") + "\n" +
              "  zeroone-tx [options] -create [commands]   " + _("Create hex-encoded zeroone transaction") + "\n" +
              "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded zeroone transaction
