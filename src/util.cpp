...
const char * const BITCOIN_CONF_FILENAME = "zeroone.conf";
const char * const BITCOIN_PID_FILENAME = "zerooned.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "zeroone" is a composite category enabling all ZeroOne-related debug output
            if(ptrCategory->count(string("zeroone"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ZeroOneCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ZeroOneCore
    // Mac: ~/Library/Application Support/ZeroOneCore
    // Unix: ~/.zeroonecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ZeroOneCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ZeroOneCore";
#else
    // Unix
    return pathRet / ".zeroonecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty zeroone.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override zeroone.conf
