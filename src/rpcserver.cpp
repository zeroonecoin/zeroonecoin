...
UniValue stop(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop ZeroOne Core server.");
    // Event loop will exit after current HTTP requests have been handled, so
    // this reply will get back to the client.
    StartShutdown();
    return "ZeroOne Core server stopping";
}
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* ZeroOne features */
    { "zeroone",               "masternode",             &masternode,             true  },
    { "zeroone",               "masternodelist",         &masternodelist,         true  },
    { "zeroone",               "masternodebroadcast",    &masternodebroadcast,    true  },
    { "zeroone",               "gobject",                &gobject,                true  },
    { "zeroone",               "getgovernanceinfo",      &getgovernanceinfo,      true  },
    { "zeroone",               "getsuperblockbudget",    &getsuperblockbudget,    true  },
    { "zeroone",               "voteraw",                &voteraw,                true  },
    { "zeroone",               "mnsync",                 &mnsync,                 true  },
    { "zeroone",               "spork",                  &spork,                  true  },
    { "zeroone",               "getpoolinfo",            &getpoolinfo,            true  },
#ifdef ENABLE_WALLET
    { "zeroone",               "privatesend",            &privatesend,            false },
...
{
    return "> zeroone-cli " + methodname + " " + args + "\n";
}
