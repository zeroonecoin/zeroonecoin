#!/usr/bin/env python
#
# Use the raw transactions API to spend zeroones received on particular addresses,
...
def read_bitcoin_config(dbdir):
    ...
    config_parser.readfp(FakeSecHead(open(os.path.join(dbdir, "zeroone.conf"))))
...
def create_tx(zerooned, fromaddresses, toaddress, amount, fee):
...
def sanity_test_fee(zerooned, txdata_hex, max_fee):
...
def main():
    ...
    parser.add_option("--from", dest="fromaddresses", default=None,
                      help="addresses to get zeroones from")
    parser.add_option("--to", dest="to", default=None,
                      help="address to get send zeroones to")
    ...
    if options.amount is None:
        ...
        txdata = create_tx(zerooned, options.fromaddresses.split(","), options.to, amount, fee)
        sanity_test_fee(zerooned, txdata, amount*Decimal("0.01"))
        if options.dry_run:
            print(txdata)
        else:
